...
class Update(object):
    ...
    def start(self):
        ...
        for repo in sorted(repos):
            ...
            template = '{0}{1}/'
            # download local PACKAGES.TXT/SLACKBUILDS.TXT if not exist,
            # check size local and remote PACKAGES.TXT/SLACKBUILDS.TXT otherwise
            if (not path.isfile(repo_file) or
                    not self.check_file_size(repo_file, repo_url)):
                prefix_repo = template.format(self.spman_conf['REPOS_PATH'],
                                              repo)
                download(repo_url, prefix_repo)

            # download local ChangeLog.txt if not exist
