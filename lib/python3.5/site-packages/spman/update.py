...
class Update(object):
    ...
    def start(self):
        ...
        for repo in sorted(repos):
            ...
            if not path.isfile(repo_file) or not path.isfile(log_file):
                self.download_both(repo, repo_url, log_url)
                continue
    ...
    def download_both(self, repo, repo_url, log_url):
        """
        download PACKAGES.TXT (SLACKBUILDS.TXT) and ChangeLog.txt
        """
        template = '{0}{1}/'
        prefix = template.format(self.spman_conf['REPOS_PATH'], repo)
        download(repo_url, prefix)
        prefix = template.format(self.spman_conf['LOGS_PATH'], repo)
        download(log_url, prefix)
