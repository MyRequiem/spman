...
{2} -v, --check-version{3}   Check program version for update
...
{2} -g, --check-upgrade{3}   Check all installed packages for upgrade
...
{2} -q, --queue{3}           Download, build and install package(s) in the
                         queue from 'sbo' repository
{2}     --add pkglist{3}         add package(s) to the queue
{2}     --remove{3}              remove package(s) from the queue
...
{2} -a, --bad-links{3}       Search for links to non-existent files/dirs
