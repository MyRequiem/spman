...
class Update(object):
    ...
    def start(self):
        ...
        for repo in sorted(repos):
            ...
            if not path.isfile(repo_file) or not path.isfile(log_file):
                self.download_both(repo_file, log_file, repo_url, log_url)
                continue
    ...
    def download_both(self, rfile, lfile, rurl, lurl):
        """
        download PACKAGES.TXT (SLACKBUILDS.TXT) and ChangeLog.txt
        """
        self.download(rfile, rurl)
        self.download(lfile, lurl)
    ...
    def download(self, dfile, url):
        ...
        print('{0}Downloading:{1} {2}'.format(self.meta.clrs['lyellow'],
                                              self.meta.clrs['reset'],
                                              url))
        ...
        if not path.isfile(dfile):
            print('{0}Failed download file !!!{1}'.format(
                self.meta.clrs['red'], self.meta.clrs['reset']))
