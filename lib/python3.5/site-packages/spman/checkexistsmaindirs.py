...
def check_exists_main_dirs():
    ...
    for repo in repos:
        ...
        if (not os.path.isfile(spman_conf['REPOS_PATH'] + repo +
                               '/' + pkg_file) or
                not os.path.isfile(spman_conf['LOGS_PATH'] + repo +
                                   '/ChangeLog.txt')):
            print(('For some repositories missing files PACKAGES.TXT, '
                   'SLACKBUILDS.txt or ChangeLog.txt\n\'spman -u\' or '
                   '\'spman --update\' for update libraries.'))
            raise SystemExit
