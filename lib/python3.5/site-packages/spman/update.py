...
class Update(object):
    ...
    def start(self):
        ...
        for repo in sorted(repos):
            ...
            # check exist local log and repo files
            ...
            if not repo_not_exist or not log_not_exist:
                print('{0}Wait...{1}'.format(self.meta.clrs['grey'],
                                             self.meta.clrs['reset']))

            # check size local and remote PACKAGES.TXT/SLACKBUILDS.TXT
            if (not repo_not_exist and
                    not self.check_file_size(repo_file, repo_url)):
                download(repo_url, prefix_repo)

            # check size local and remote ChangeLog.txt
            if (not log_not_exist and
