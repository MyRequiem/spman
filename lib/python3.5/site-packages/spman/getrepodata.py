...
class GetRepoData(object):
    ...
    def __init__(self, reponame):
        ...
        self.rdata = {
            ...
            'pkgs': {
                # 'pkg_name': [
                #   [0] - package version
                #   [1] - location packages (only for sbo repo)
    ...
    def get_non_sbo_data(self, line):
        ...
        if line.startswith('PACKAGE NAME: '):
            ...
            # package version
            self.rdata['pkgs'][self.pkgname][0] = parts[1]
        elif line.startswith('PACKAGE SIZE (compressed): '):
            self.process_size_pkg(line, self.pkgname, 0)
        elif line.startswith('PACKAGE SIZE (uncompressed): '):
            self.process_size_pkg(line, self.pkgname, 1)
        elif line.startswith('PACKAGE REQUIRED: '):
            self.get_req_pkg(line, self.pkgname, ',')
    ...
    def get_req_pkg(self, line, pkgname, sep=None):
        """
        ...
        for req in str_req:
            if req:
                self.rdata['pkgs'][pkgname][4].append(req.strip())

    def process_size_pkg(self, line, pkgname, ind):
        """
        docstring
        """
        # if ind == 0 compressed size, else uncompressed
        val = int(self.get_line_value(line, ': ').split()[0].strip())
        self.comp[ind] += val
        hval = self.get_human_readable_size(val)
        self.rdata['pkgs'][pkgname][ind + 2] = hval
